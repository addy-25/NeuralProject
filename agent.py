...
class Agent:
    ...
    def sense(self, foods, others):
        ...
        # foods can be either (x,y) tuples (for prey) or Agent objects (for predators)
        best_fd = 9999
        fdx, fdy, fd_norm = 0.0, 0.0, 1.0

        for f in foods:
            # Support both (x,y) tuples and Agent objects
            fx = f.x if isinstance(f, Agent) else f[0]
            fy = f.y if isinstance(f, Agent) else f[1]
            # Skip dead agents
            if isinstance(f, Agent) and not f.alive:
                continue
            d = math.hypot(fx - self.x, fy - self.y)
            if d < best_fd:
                best_fd = d
                # Unit vector pointing toward food/target
                fdx = (fx - self.x) / max(d, 1)
                fdy = (fy - self.y) / max(d, 1)
                fd_norm = min(d / W, 1.0)
    ...
    def update(self, foods, threats, targets):
        ...
        dx, dy = self.brain.forward(inputs)
